...
# Keep existing User and Chat models
class User(db.Model):
    __tablename__ = 'users'
...
def chat():
    ...
    try:
        if request.method == 'POST':
            # Handle AJAX POST requests from chat.html
            ...
            # Handle GET requests (initial load and queries from ghat.html)
            query = request.args.get('query', '')
            chat_history = db.session.query(Chat).filter(Chat.user_id == user_id).order_by(Chat.timestamp.desc()).all()
            
            if query:
                # If there's a query in the URL (from ghat.html)
                response_data = process_search_query(query, user_id)
            ...
            # Just show chat history if no query
